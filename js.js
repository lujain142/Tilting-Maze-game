...
// يتم تعريف دالة Math.minmax التي تستخدم لإرجاع القيمة الأصغر أو الأكبر بين قيمة معينة وحدود معينة.
// تعريف دالة distance2D التي تستخدم لحساب المسافة بين نقطتين في مستوى ثنائي الأبعاد باستخدام معادلة المسافة بين نقطتين.
// تعريف دالة getAngle التي تستخدم لحساب الزاوية بين نقطتين باستخدام دالة الجيب التمامية وتعديل الزاوية بناءً على الربع الذي تقع فيه النقطتين.
// تعريف دالة closestItCanBe التي تستخدم لحساب أقرب موقع يمكن أن يكون عليه الكرة بالنسبة لحافة جدار.
// تعريف دالة rollAroundCap التي تستخدم لحساب حركة الكرة حول حافة الجدار باستخدام الزوايا والمسافات والسرعة.
// تعريف دالة slow التي تستخدم لتقليل قيمة رقمية معينة ولكن بالحفاظ على العلامة السالبة أو الموجبة وعدم الانخفاض دون الصفر المطلق.
// تم تعريف متغيرات وعناصر HTML المستخدمة في اللعبة.
// تم تعريف ثوابت لعرض الممرات وعرض الجدران وحجم الكرة وحجم الثقب.
// تم إعادة تعيين اللعبة باستخدام الدالة resetGame.
// تم رسم الكرات للمرة الأولى باستخدام المتغيرات balls وballElements.
// تم تعريف مصفوفة walls التي تحتوي على معلومات الجدران في المتاهة.
  ...
  function resetGame() {
    ...
    // يتم استخدام دالة forEach للتكرار على كل عنصر في مصفوفة walls.
    // يتم إنشاء عنصر div جديد يمثل الجدار باستخدام document.createElement("div").
    // يتم تعيين الفئة "wall" للعنصر الجديد باستخدام wall.setAttribute("class", "wall").
    // يتم تعيين الخصائص CSS للعنصر الجديد مثل الإحداثيات والعرض والارتفاع والدوران باستخدام خصائص style.cssText.
    // يتم إضافة العنصر الجديد كابن لعنصر mazeElement الذي يمثل المتاهة.
    // يتم تحويل المصفوفة holes إلى مصفوفة من الكائنات التي تحتوي على إحداثيات الثقوب في المتاهة.
    // يتم استخدام حدث "mousedown" للتحقق مما إذا كان المستخدم قد قام بالنقر على عنصر joystickHeadElement، وفي حالة النجاح، يتم تعيين بعض المتغيرات وبدء اللعبة واستدعاء دالة main باستخدام window.requestAnimationFrame.
    // يتم استخدام حدث "mousemove" لتحديث موقع عنصر joystickHeadElement بناءً على حركة الماوس، ويتم حساب دوران المتاهة وتعيين القوى المؤثرة على الكرة بناءً على حركة الماوس.
    // يتم استخدام حدث "keydown" للتحقق من الزر المضغوط، وفي حالة النجاح، يتم إعادة تعيين اللعبة أو ضبط الوضع الصعب أو ضبط الوضع السهل بناءً على الزر المضغوط.
    // تعريف دالة resetGame التي تقوم بإعادة تعيين اللعبة إلى حالتها الأولية بما في ذلك إعادة تعيين المتغيرات وتحديث العناصر الخاصة بالمتاهة والكرات والإشعارات.
...
//           دالة resetHoles(hardMode):

// تقوم بإزالة العناصر السابقة التي تم إنشاؤها لثقوب المتاهة.
// تقوم بإعادة ضبط المتغيرات المرتبطة بثقوب المتاهة إذا كانت اللعبة في وضع صعوبة عالية (hard mode).
// تقوم بإنشاء عناصر جديدة لثقوب المتاهة وإضافتها إلى عنصر المتاهة في حالة الوضع العالي للصعوبة.
// دالة main(timestamp):

// تمثل الدورة الرئيسية للعبة وتستدعى باستمرار بواسطة window.requestAnimationFrame.
// تقوم بحساب الوقت المنقضي منذ الدورة السابقة باستخدام الطابع الزمني (timestamp).
// تقوم بتحديث حركة الكرات في المتاهة استنادًا إلى القوى المسارية والاحتكاك.
// تقوم بالتصادم والاندفاع للكرات مع الحوائط داخل المتاهة.
      ...
      if (
        ...
        noteElement.innerHTML = `Congrats, you did it!
          ...
          <p>
            Follow me
            <a href="https://www.linkedin.com/in/lujain-al-ramadan/" , target="_blank"
              >@Lujain-al-ramadan</a
            >
          </p>`;
        noteElement.style.opacity = 1;
...
// يتم استدعاء هذا الجزء في الدورة الرئيسية للعبة (main) لفحص ما إذا كانت الكرة قد سقطت في أحد الثقوب.
// إذا كان الوضع صعبًا (hardMode = true)، فسيتم فحص كل ثقب في المتاهة باستخدام دالة forEach.
// يتم حساب المسافة بين موقع الثقب وموقع الكرة المتوقع بالاستعانة بدالة distance2D، وهي دالة غير معروفة في الكود المقدم.
// إذا كانت المسافة أقل من أو تساوي نصف حجم الثقب (holeSize / 2)، فهذا يعني أن الكرة سقطت في الثقب.
// يتم تغيير لون العنصر المرتبط بالثقب في واجهة المستخدم إلى اللون الأحمر لإظهار أن الكرة سقطت فيه.
// يتم رمي خطأ (throw Error) للإشارة إلى أن الكرة سقطت في ثقب.
// بعد ذلك، يتم تحديث بيانات الكرة وتحريكها إلى المو